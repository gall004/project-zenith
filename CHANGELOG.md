...
### Fixed
- Bug `fix/session-routing-collision`: Fixed a critical routing collision where the `request_visual_context` OpenAPI webhook would fall back to the first active WebSocket room when `session_id` was omitted by the orchestrator, causing video sessions to trigger on the wrong user's device. The webhook now routes to the most recently active room based on chat activity timestamps.
- Bug `fix/mobile-mic-sensitivity`: Configured `LiveKitRoom` with explicit `audioCaptureDefaults` (enabling `autoGainControl`, `echoCancellation`, and `noiseSuppression`) to resolve low microphone sensitivity and force proper speakerphone audio routing on iOS Safari devices.
- Bug `fix/github-secret-leak`: Scrubbed hardcoded API keys leaked in `scripts/deploy-prod.env` and correctly added `scripts/deploy-*.env` to `.gitignore`. Updated `dod-check` governance rule 6 to actively scan the `scripts/` directory, preventing future config leaks.
